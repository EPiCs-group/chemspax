...
echo "Modified by Mark Heezen"
echo "---------------------------------------------------------------------------------------------"

# ChemSpaX loads the input into bash arrays
A=($2)
B=($3)
...
# The first substituent is read as the first and only element of the array. Directly reading it as a string gave errors. 
STARTING_C_SUBSTITUENT=${A[0]}
...
#if you do not want a random substituent, comment the above line and uncomment the line below
RANDOM_C_SUBSTITUENTS=${B[@]}

function join { local IFS="$1"; shift; echo "$*"; }
RANDOMCSUBS=$(join _ ${RANDOM_C_SUBSTITUENTS[@]})
...
cd skeletons
tempfilename=temp_skeletons_file_${STARTING_C_SUBSTITUENT}_${RANDOMCSUBS}
ls *.xyz > ../${tempfilename}
cd -
N=$(wc -l ${tempfilename} | cut -d' ' -f1)
declare -a TARGETNAMES=()

for j in $(seq 1 ${N}); do
# loop over skeleton list and set index back to 1
    skeleton=$(head -${j} ${tempfilename} | tail -1 | cut -d'.' -f1)
    SOURCE_FILE=skeletons/${skeleton}
    TARGET_NAME=${skeleton}_func_${STARTING_C_SUBSTITUENT}_${RANDOMCSUBS}
    i=1
    ...
    echo file_name,substituent,index,functional_group > ${TARGET_NAME}_funcs_map.csv
    echo ${TARGET_NAME}_${i},${STARTING_C_SUBSTITUENT},X1,R1 >> ${TARGET_NAME}_funcs_map.csv
    cd -
        ...
        TARGETNAMES[${#TARGETNAMES[@]}]=${TARGET_NAME}_${i}
        ...
        if [ $((i+1)) -le 5 ]
        then
            echo ${TARGET_NAME}_$((i+1)),${sub},X$((i+1)),R1 >> ${TARGET_NAME}_funcs_map.csv
        elif [ $((i+1)) -gt 5 ] && [ $((i+1)) -le 10 ]
        then
            echo ${TARGET_NAME}_$((i+1)),${sub},X$((i+1)),R2 >> ${TARGET_NAME}_funcs_map.csv
        elif [ $((i+1)) -gt 10 ] && [ $((i+1)) -le 15 ]
        then
            echo ${TARGET_NAME}_$((i+1)),${sub},X$((i+1)),R3 >> ${TARGET_NAME}_funcs_map.csv
        elif [ $((i+1)) -gt 15 ] && [ $((i+1)) -le 20 ]
        then
            echo ${TARGET_NAME}_$((i+1)),${sub},X$((i+1)),R4 >> ${TARGET_NAME}_funcs_map.csv
        else
            echo ${TARGET_NAME}_$((i+1)),${sub},X$((i+1)),R0 >> ${TARGET_NAME}_funcs_map.csv
        fi
	    cd -
        i=$((i+1))
        done
    
    #deleting all intermediate stages
    cd substituents_xyz/automatically_generated
    rm -f ${TARGET_NAME}_funcs_map.csv
    for j in $(seq 0 $((${#TARGETNAMES[@]}-1))); do
        rm -f ${TARGETNAMES[j]}.xyz
        rm -f ${TARGETNAMES[j]}.mol
    done
    
    # Rename the last files to files without a number
    mv  ${TARGET_NAME}_${i}.xyz ${TARGET_NAME}.xyz
    mv  ${TARGET_NAME}_${i}.mol ${TARGET_NAME}.mol
    cd -
done
