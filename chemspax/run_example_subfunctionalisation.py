...
from chemspax.subfunctionalisations import initialise, sub_sites_value_length, remove_dummy_atom_MOL, remove_dummy_atom_XYZ
from chemspax.pick_subfunctionalisations import H_alkyl_aryl
from chemspax.run_subfunctionalisation import check_skeleton_sites
...
def final_functionalisation(skeleton):
    ...
    path_to_database = os.path.join(path_to_substituents, "central_atom_centroid_database.csv")
    path_to_skeletons = os.path.join(working_directory, "skeletons_test")
    skeleton_list = glob.glob(os.path.join(working_directory, "skeletons_test", str(list(func_sites.keys())[skeleton])+'.xyz'))
...
if __name__ == "__main__":
    ...
    prepare_data(path_to_substituents, path_to_skeletons, path_to_database)
    func_sites=check_skeleton_sites('skeletons_test')
