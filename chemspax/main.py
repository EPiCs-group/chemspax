...
def main(
    ...
    logger.log(
        logging.INFO,
        f"ChemSpaX working directory is set to {working_directory}",
    )
    ...
    if not skeleton_list:
        ...
        logger.log(
            logging.INFO,
            f"{len(skeleton_list)} skeletons provided to functionalize",
        )
    ...
    for skeleton in skeleton_list:
        ...
        logger.log(
            logging.INFO,
            f"Attaching substituents to skeleton: {skeleton_name}",
        )
        ...
        if substituent_list is not None:
            ...
            len_functionalization_list = len(
                complex.functionalization_site_list
            )
        ...
        if len(complex.functionalization_site_list) == 0:
            logger.log(
                logging.INFO,
                f"No functionalization sites found for {skeleton_name}",
            )
            continue
        ...
        for idx, substituent in enumerate(substituent_list[1:]):
            ...
            new_skeleton_path = os.path.join(
                path_to_output, new_skeleton_name + ".xyz"
            )
        ...
        logger.log(
            logging.INFO, f"Attached substituents to skeleton: {skeleton_name}"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="chemspax",
        description="Attach substituents to a skeleton molecule",
    )
    parser.add_argument(
        "-s",
        "--substituent",
    ...
    if args.skeleton is not None:
        ...
        list_of_skeleton_files = glob.glob(
            os.path.join(args.skeleton, "*.xyz")
        )
    else:
        list_of_skeleton_files = glob.glob(
