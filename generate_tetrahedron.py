...
class Complex:
    def __init__(self, path_to_data):
        ...
        self.atom_to_be_functionalized_index = 1  # index in .xyz file of atom to be functionalized
        ...
        self.bonded_atom_index = 0  # index in .xyz file of atom bonded to atom to be functionalized
    ...
    def generate_substituent_and_write_xyz(self, filename, substituent_1_atom, substituent_2_atom, substituent_3_atom,
        ...
        if not path.exists(filename):
            ...
            # increae n_atoms correctly
            with open(filename) as f:
                lines = f.readlines()
                lines[0] = int(lines[0]) + 5
                lines[0] = str(lines[0])+'\n'
...
if __name__ == '__main__':
    # ipr = Complex('substituents_xyz/manually_generated/iPr.xyz')
    # ipr.atom_to_be_functionalized_index = 9
    # ipr.bonded_atom_index = 10
    # ipr.generate_substituent_and_write_xyz('test', 'H', 'H', 'H', False, False)
    # ipr.atom_to_be_functionalized_index = 1
    # ipr.bonded_atom_index = 0
    # ipr.generate_substituent_and_write_xyz('test', 'H', 'H', 'H', True, False)
    # example usage for methyl ToDo: substituent atoms end up at the same position as in previous file (??)
    methyl = Complex('substituents_xyz/manually_generated/CH3.xyz')
    methyl.generate_substituent_and_write_xyz('CH4', 'H', 'H', 'H', False, False)
    ethyl = Complex('substituents_xyz/automatically_generated/CH4.xyz')
    ethyl.atom_to_be_functionalized_index = 4
    ethyl.bonded_atom_index = 1
    ethyl.generate_substituent_and_write_xyz('CH4', 'C', 'C', 'C', True, False)

    # manually print vectors
