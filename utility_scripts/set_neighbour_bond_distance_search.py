...
def set_neighbour_bond_distance_search(source_mol_file, central_atom_atomic_num, search_this_atomic_num,
                                       new_length, new_atomic_num):
    ...
    for atom in openbabel.OBMolAtomIter(mol):
        ...
        if atomic_number == central_atom_atomic_num:  # central atom is found
            ...
            for neighbour_atom in openbabel.OBAtomAtomIter(atom):
                ...
                if atomic_number == search_this_atomic_num:  # atom that you were searching for was found
                    search_this_atom_vector = np.array([neighbour_atom.x(), neighbour_atom.y(), neighbour_atom.z()])
                    d = neighbour_atom.GetDistance(atom)
                    distance_vector = (search_this_atom_vector - central_atom_vector)/d
                    new_vector = scale(central_atom_vector, distance_vector, new_length)
                    neighbour_atom.SetAtomicNum(new_atomic_num)
                    neighbour_atom.SetVector(new_vector[0], new_vector[1], new_vector[2])
...
if __name__ == '__main__':
    ...
    set_neighbour_bond_distance_search(source_file, central_atom_atomic_number, atomic_number_to_look_for,
                                       new_bond_length, new_atomic_number)
