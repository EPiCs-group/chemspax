...
class Complex:
    ...
    def generate_substituent_and_write_xyz(self, filename, bonded_atom_sub_0_length, substituent_0_atom,
                                           sub_0_sub_1_length, substituent_1_atom,
                                           sub_0_sub_2_length, substituent_2_atom,
                                           sub_0_sub_3_length, substituent_3_atom):  # view_file=False, save_file=False):
        folder = '../substituents_xyz/automatically_generated/'
        extension = '.xyz'
        filename = folder + filename + extension
        v_1, v_2, v_3 = self.generate_substituent_vectors(sub_0_sub_1_length, sub_0_sub_2_length, sub_0_sub_3_length)
        ...
        if self.recursive_or_initial.lower() == 'initial':
            ...
            # read atom_to_be_functionalized as df
            ...
            # replace atom with sub0 atom and xyz values with calculated values
            atom_to_be_functionalized_data['atom'] = substituent_0_atom
            atom_to_be_functionalized_data['x'] = str(new_bond_functionalized_and_bonded_atom[0])
            atom_to_be_functionalized_data['y'] = str(new_bond_functionalized_and_bonded_atom[1])
            atom_to_be_functionalized_data['z'] = str(new_bond_functionalized_and_bonded_atom[2])
            # write .values to correctly to line in file s.t. no problems occur with recursion
            atom_to_be_functionalized_str = str(atom_to_be_functionalized_data.values).\
            ...
            with open(self.path) as f:
                ...
                # replace atom_to_be_functionalized with substituent 0
                lines[self.atom_to_be_functionalized_index+2] = atom_to_be_functionalized_str+'\n'
